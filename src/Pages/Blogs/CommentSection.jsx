  ...
  return (
    <Box mt={4} mb={4}>
      ...
      {comments.slice().reverse().slice(initialCommentLimit,commentLimit).map((comment) => (
       ...
       <Button variant="contained" onClick={handleLoadMore} style={{marginLeft: "1rem",marginTop:"1rem"}}>{commentLimit===0?<><MessageIcon/> View Comments({comments.length})</>:"Load More"}</Button>
    </Box>
  );
};
